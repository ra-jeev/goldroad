 ...
 *
 * Design note: Board stores only non-open edge groups (missing, toll, bonus).
...
export function buildEdgeMap(board: Board): EdgeMap {
  ...
  for (const edge of board.missingEdges) {
    map.set(edgeKey(edge.from, edge.to), 'missing');
    map.set(edgeKey(edge.to, edge.from), 'missing');
  }

  for (const edge of board.tollEdges) {
    map.set(edgeKey(edge.from, edge.to), 'toll');
    map.set(edgeKey(edge.to, edge.from), 'toll');
  }

  for (const edge of board.bonusEdges) {
 ...
 * Missing edges prevent movement; toll and bonus edges remain traversable.
...
export function getNeighborId(
  ...
  switch (dir) {
    case 'top':
      return row > 0 ? tileIndex(row - 1, col, cols) : null;
    case 'bottom':
      return row < rows - 1 ? tileIndex(row + 1, col, cols) : null;
    case 'left':
      return col > 0 ? tileIndex(row, col - 1, cols) : null;
    case 'right':
      return col < cols - 1 ? tileIndex(row, col + 1, cols) : null;
  }
}
 ...
 */
export function allBoardEdgePairs(
  rows: number,
  cols: number,
): Array<[number, number]> {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (c < cols - 1)
        pairs.push([tileIndex(r, c, cols), tileIndex(r, c + 1, cols)]); // horizontal
      if (r < rows - 1)
        pairs.push([tileIndex(r, c, cols), tileIndex(r + 1, c, cols)]); // vertical
    }
  }
  return pairs;
}
