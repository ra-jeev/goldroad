...
import type { Direction, Medal, PuzzleType } from '../../../shared/types/game';
...
type ReplayRoadDay = Awaited<ReturnType<typeof gamesApi.getGameBoard>>;
type ReplayGame = NonNullable<ReplayRoadDay['classic']>;

const availableGames = ref<ReplayRoadDay>({
  classic: null,
  expedition: null,
});
...
const solvedBadge = computed(() => {
  ...
  return lastMedal.value
    ? UI_COPY.boardFooter.medalAwarded(
        UI_COPY.boardHeader.medals[lastMedal.value],
      )
    : 'Solved';
});
...
function setBodyMode(mode: PuzzleType | null) {
...
function setupGame(
  ...
  const preservedHintsUsed = options.preserveSession ? hintsUsed.value : 0;
...
async function loadReplayGame() {
  ...
  if (!Number.isInteger(gameNo) || gameNo <= 0) {
    ...
    loading.value = false;
    availableGames.value = { classic: null, expedition: null };
  ...
  try {
    ...
    const preferredMode =
      selectedMode.value === 'expedition' && roadDay.expedition
        ? 'expedition'
        : roadDay.classic
          ? 'classic'
          : roadDay.expedition
    ...
    if (!preferredMode) {
      throw new Error('No archived game variants available for this road.');
    }
...
async function finalizeRun(
  ...
  if (!game.value || submitting.value || !playerUUID.value || !sessionId.value)
    return;
...
async function requestHint() {
  if (!game.value || ended.value || !playerUUID.value) return;
        ...
        <section class="archive-footer">
          ...
          <div class="action-row">
            ...
            <button
              type="button"
              class="ghost"
              @click="useHowToPlaySheet().openHowToPlay()"
            >
