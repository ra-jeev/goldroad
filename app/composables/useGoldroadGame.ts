...
export function useGoldroadGame() {
  ...
  function markClassicSolved(solved: boolean) {
    if (typeof window === 'undefined' || selectedMode.value !== 'classic')
      return;
  ...
  function setupGame(
    ...
    attemptNumber.value =
      options.attemptNumber ?? progressAttemptNumber ?? nextAttemptNumber;
  ...
  async function loadAvailableGames() {
    ...
    try {
      ...
      setupGame(
        preferredMode === 'classic' ? response.classic! : response.expedition!,
      );
    } finally {
      loading.value = false;
    }
