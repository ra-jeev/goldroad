...
- **Today's solved card stays outside mode tabs and grows with the day**: no solve invites Classic; a Classic-only result shows one medal/details row, remains shareable, and invites Expedition; two results show both rows and share the combined day. The medal "+1" is golden text (v1's), never a green badge, and the medal count bumps on arrival.
...
- **The today result block uses the same compact visual grammar as day completion**: medal/check icon, mode title, then `tries · time` and nonzero hints. Share text still carries the medal emoji (🥇🥈🥉, 😅 for late solves), where platform rendering is out of our hands anyway.
...
- Acceptance criteria:
  ...
  - percentile claims use the exact solved-attempt distribution; the later July 2026 phone-play review deliberately removed the minimum field-size gate
...
- Completion notes:
  ...
  - the initial implementation added a 10-solver percentile gate; the July 2026 phone-play review superseded it, so the histogram now appears from the first play and the percentile from the first recorded solve
...
- Progress notes:
  ...
  - pooled-percentile proof (`tests/statsPresentation.test.ts`): extracted `stats.vue`'s `topPercent`/`toPercent`/`hasPercentileSample` into a new pure module `app/utils/statsPresentation.ts` (page behavior unchanged, now imports from it) plus a new `hasCommunitySample` helper; a 30-solver fixture (1..29 attempts once each, plus one solver at 40) proves the exact percentile stays correct for a player exactly at the pooled 25+ boundary, past it (attempt 40, correctly 100% rather than a bucket artifact), and at the interior edge (attempt 30) — cross-checked against a manual sum over the exact map, independent of `buildSolvedAttemptsDistribution`'s pooled bucket
  ...
  - stats sparse-data states (`tests/statsPresentation.test.ts`): `hasCommunitySample` and `hasPercentileSample` are exported pure predicates and test the true empty-data boundaries; histogram presentation starts with the first play and percentile presentation with the first recorded solve
...
- Completion notes:
  ...
  - yesterday's narrative uses v1's voice: "X% of the roadgoers who walked down Road N reached the finish" and "You got to the finish in N attempts — in the top Y% of the field" (v1's at-or-better percentile); the histogram appears from the first play, the percentile from the first recorded solve, and a zero-play road keeps its card with an explicit empty state
...
- Completion notes:
  ...
  - shared audio explicitly resumes from the first eligible gesture, queues the initially requested effect until ready, and preserves mute suppression; after an installed-iPhone lock/unlock test exposed a stale recovery promise, foreground transitions now invalidate the prior attempt and defer WebKit-safe recovery to the next real gesture; device retest remains open until deployment
  ...
  - Stats now has a proper separated arrow marker, `tries` axis/copy, direct hint-total and solve-time comparison, selected-mode streak below the common segmented control, larger token-colored flame, single-line `1 TRY`/`2 TRIES`/`3 TRIES` labels, and count-only medal animation; day-complete celebration rows use one icon column and one mode/details column, omit the repeated written medal, and show only the next-road countdown without a redundant UTC label
...
### Issue SF-F1 — July 22 installed-app and open-page follow-up

- Priority: `P0`/`P1`
- Status: `implemented locally; iPhone lock/unlock retest pending deployment`
- Completion notes:
  - foreground audio recovery invalidates stale unlock work and resumes on the next eligible gesture, preserving autoplay and mute constraints
  - `loadNewRoad` applies an authoritative new road directly; it no longer lets the expired old board's mode-switch guard leave new tabs mounted over stale game state
